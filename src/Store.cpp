...
Store::Store()
{
    Vector2f pos = Math::getPositionFromIndex(64);
    this->updatePosition(pos);
    this->m_rect.x = getPosition().m_x;
    this->m_rect.y = getPosition().m_y;
...
Vector2f     Store::getStandardTowerPos() const
{
    return m_stdTowerShopPos;
}

Vector2f     Store::getSlowingTowerPos() const
{
    return m_slwTowerShopPos;
}

Vector2f     Store::getExplosiveTowerPos() const
{
...
int         Store::getPrice(std::string towerName) const
{
    /**
    *   REVIEW:
    *   Utilise une enum au lieu d'une string.
    */
